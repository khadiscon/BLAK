        ...
        >
          {items.map((item) => (
            <button
              ...
              className="w-full text-left block px-4 py-3 text-sm text-stone-700 hover:bg-green-50 hover:text-green-700 font-medium transition-colors"
            >
              {item.name}
            </button>
          ))}
              ...
              if ('dropdown' in item && item.dropdown) {
                ...
                return (
                  <div key={item.name} className="relative">
                    ...
                    <DropdownMenu items={item.dropdown} visible={desktopDropdown === item.name} onClose={() => setDesktopDropdown(null)} />
                  </div>
                );
              }
              return (
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (sub.path.includes('#')) {
                                  const [route, hash] = sub.path.split('#');
                                  const targetRoute = route || '/';
                                  if (location.pathname === targetRoute) {
                                    scrollToId(hash);
                                  } else {
                              ...
                              className="w-full text-left block py-3.5 px-5 text-sm font-medium text-stone-600 hover:text-green-700 border-b border-stone-100 last:border-0 transition-colors"
                            >
                              {sub.name}
                            </button>
                          ))}
